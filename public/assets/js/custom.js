 ...
 */
 ...
 function CopyToClipboard(id)
 {
    ...
    // element.innerHTML = "copyed";
 var r = document.createRange();
 r.selectNode(document.getElementById("copycode-"+id));
 window.getSelection().removeAllRanges();
 window.getSelection().addRange(r);
 document.execCommand("copy");
 window.getSelection().removeAllRanges();
//  element.append(p)
// var Transfer1 = document.getElementById(id).innerHTML;
document.getElementById("copy-"+id).innerHTML = 'copied';

setTimeout(() => {document.getElementById("copy-"+id).innerText = 'Copy'}, 3000);
...
var r = document.createRange();
r.selectNode(document.getElementById("copy"+id));

window.getSelection().removeAllRanges();
window.getSelection().addRange(r);
document.execCommand("copy");
window.getSelection().removeAllRanges();
document.getElementById("copyUrl-"+id).innerHTML = 'copied';

setTimeout(() => {document.getElementById("copyUrl-"+id).innerText = 'Copy'}, 3000);
...
$(document).ready(function() {
    ...
    $('.data-table-order-desc').DataTable({
    "ordering": false,
    ...
    $("#submit").click(function(){
         ...
         var formData = {
                 ...
                 'nextDate': nextDate,
                 'referral_code' : referral_code, //for get email
             };
         $.ajax({
             ...
             success: function(responses){
                 ...
                 $.each(responses, function(index,response){
                     $("#orders").append('<tr><td>#'+(response.order_id == null?'-':response.order_id)+'</td><td>'+(response.name == null?'-':response.name)+'</td><td>'+(response.order_date == null?'-':response.order_date)+'</td><td>'+(response.number_of_tablet == null?'-':response.number_of_tablet)+'</td><td>'+(response.commission_rate == null?'-':response.commission_rate)+'</td><td>'+(response.commission == null?'-':response.commission)+'</td></tr>');
                     total=parseInt(total)+parseInt(response.commission);
                 });
                 $("#total").text('');
                 $("#total").text(total);
                 // console.log(response);
                 //<td>'+(response.yellowcards == null?'-':response.yellowcards)+'</td>
             }
         });
     });
...
function initMap() {
    ...
    autocomplete.addListener('place_changed', function() {
        ...
        $('#doctor_lat').attr("value",place.geometry.location.lat());
        $('#doctor_lon').attr("value",place.geometry.location.lng());
        $('#postal_code1').attr("value",postal_code);
        $('#chamber_lat').attr("value",place.geometry.location.lat());
        $('#chamber_lon').attr("value",place.geometry.location.lng());
